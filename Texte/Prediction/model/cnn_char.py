...
class CharCNN(object):
    ...
    def __init__(self, network_architecture, learning_rate=0.0001, load_model=None, gradient_clip=None,
        ...
        """
            with tf.Session () as sess:
                new_saver = tf.train.import_meta_graph ('D:/Users/wissam/Documents/These/these/git_reposetory/paraphrase_sentiment_analysis/Experements/SENTIMENT_ANALYSIS_TOOLS/DocumentClassification/log/cnn_char/l0.000100_e100_b100/model.ckpt.meta')
                new_saver.restore (sess , tf.train.latest_checkpoint ('D:/Users/wissam/Documents/These/these/git_reposetory/paraphrase_sentiment_analysis/Experements/SENTIMENT_ANALYSIS_TOOLS/DocumentClassification/log/cnn_char/l0.000100_e100_b100'))
        """
        """
            with tf.Session () as sess:
        ...
        self.sess = tf.compat.v1..Session(config=tf.ConfigProto(log_device_placement=False))
        ...
        if load_model:
            ...
            #D:/Users/wissam/Documents/These/these/git_reposetory/paraphrase_sentiment_analysis/experements_python/SENTIMENT_ANALYSIS_TOOLS/DocumentClassification/log/cnn_char/l0.000100_e100_b100/model.ckpt.meta
            self.saver = tf.train.import_meta_graph ('./log/cnn_char/l0.000100_e100_b100/progress-50-model.ckpt.meta')
            #print("here2")
            #self.saver.restore (self.sess ," D:/Users/wissam/Documents/These/these/git_reposetory/paraphrase_sentiment_analysis/Experements/SENTIMENT_ANALYSIS_TOOLS/DocumentClassification/log/cnn_char/l0.000100_e100_b100/model.ckpt.index")
            #print("here3")
            self.saver.restore (self.sess , tf.train.latest_checkpoint ('./log/cnn_char/l0.000100_e100_b100'))
            ops = self.sess.graph.get_operations ()
    ...
    def _create_network(self):
        ...
        self.x_char = tf.compat.v1..placeholder(tf.float32, [None] + self.network_architecture["input_char"], name="input_char")
        self.x_word = tf.compat.v1..placeholder(tf.float32, [None] + self.network_architecture["input_word"], name="input_word")
        if self.binary_class:
            self.y = tf.compat.v1..placeholder(tf.float32, [None], name="output")
        else:
            self.y = tf.compat.v1..placeholder(tf.float32, [None, self.network_architecture["label_size"]], name="output")
        # Regularization
        ...
        _layer = tf.nn.max_pool2d(_layer, ksize=[1, __word_size, 1, 1], strides=[1, 1, 1, 1], padding='VALID')
        ...
        if self.binary_class:
            ...
            # logistic loss
            _loss = self.y * tf.math.log(self.prediction + 1e-8) + (1 - self.y) * tf.math.log(1 - self.prediction + 1e-8)
            ...
            # cross entropy
            self.loss = - tf.reduce_sum(self.y * tf.math.log(self.prediction + 1e-8))
            # accuracy
        ...
        self.lr_decay = tf.compat.v1..placeholder_with_default(1.0, [])
        optimizer = tf.compat.v1..train.AdamOptimizer(self.learning_rate / self.lr_decay)
    ...
    def embedding_char(self, x, embed_dim_c=5, embed_dim_w=10, wk=5):
        ...
        embedding = tf.nn.max_pool2d(embedding, ksize=[1, 1, __shape[2], 1], strides=[1, 1, 1, 1], padding='VALID')
