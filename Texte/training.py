...
def train(epoch, model, feeder, input_format, save_path="./", lr_decay=1.0, test=False):
    ...
    tf_writer = tf.compat.v1.summary.FileWriter(save_path, model.sess.graph)
    ...
    for _e in range(epoch):
        ...
        logger.info("epoch %i: acc %0.3f, loss %0.3f, train acc %0.3f, train loss %0.3f"
                    % (_e, _result_full[2], _result_full[2], _result_full[1], _result_full[0]))
...
def get_options(parser):
    ...
    parser.add_argument('-d', '--decay_lr', help='Decay index for learning rate (default: 1.0)',
                        default=1.0, type=float, **share_param)
    parser.add_argument('-t', '--train_path', help='Decay index for learning rate (default: 1.0)',
                        default="./data/stanfordSentimentTreebank",  **share_param)
...
if __name__ == '__main__':
    ...
    # load data
    data = data_set(args.train_path)
    print("here")
    # load model
    model_instance = model.get_model_instance(model_name=args.model,
    ...
    print("in data feeder")
    
    # train
    data_feeder = model.BatchFeeder(data["sentence"], data["label"], batch_size=args.batch,
                                                validation=0.05, process=model_instance["processing"])
    
    print("out data feeder")
